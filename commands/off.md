...
2. **Stop calling `mcp__tts__say`** for the rest of this session — the earlier `/tts-mcp:on` instruction is now superseded.
...
This is per session — it deletes only *this* session's file. A concurrent session with its own `/tts-mcp:on` is unaffected, which is the point of keying the state on the session id rather than a project-wide setting.
