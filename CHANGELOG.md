...
## Unreleased
- voice skill: document the wedged-relay failure mode (`health_check_unreachable` from the tool while `curl` reaches the server) and point it at `/mcp` rather than a server restart
