...
- **`sender` is a fallback, not the label.** The relay resolves your Claude Code session name itself — from the session registry, via the `claude` ancestor process it is spawned under — and *that* name is what the web UI (http://127.0.0.1:12000/) and `GET /state` display. Pass `sender` as a short tag (anchor task → parent goal → repo/service) for the one case the relay cannot resolve a name: a session with no registry entry, where your value becomes the displayed label. Never send the session id or a full task title. An utterance is unattributable only when the session has no registry entry *and* no `sender` was passed — so passing it is still cheap insurance, but it no longer decides the label.
